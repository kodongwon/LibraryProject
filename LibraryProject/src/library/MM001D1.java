...
public class MM001D1 extends JDialog
{
	JPanel panel;
	JDialog dialog;
	...
	void setObject()
	{
		...
		btnDelete.setBounds(50, 550, 300, 30);
		//버튼 이벤트 추가
	...
	ActionListener ClickbtnSave = new ActionListener() 
	{
		...
		{
			if(MM001.stats.equals("U"))
			{
				sqlc.UpdateMember(id.getText(), new String(pw.getPassword()), name.getText(), email.getText(), phone.getText(), kind.getText());
				JOptionPane.showMessageDialog(null, id.getText()+ " 님 정보수정완료 ");
				MM001.dtm.setRowCount(0);
				sqlc.MemberInfo();
				dialog.dispose();
			}
			else
	...
	{		
		...
		public void actionPerformed(ActionEvent e) 
		{
			sqlc.DeleteMember(id.getText());
			JOptionPane.showMessageDialog(null, id.getText() + " 삭제완료 ");
			MM001.dtm.setRowCount(0);
			sqlc.MemberInfo();
			dialog.dispose();
		}
